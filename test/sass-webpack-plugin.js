...
const StylePlugin = require('../src').default;
...
describe('StylePlugin', function() {
  describe('#constructor', function() {
    it('handles file case', function(done) {
      let plugin = new StylePlugin('demo/src/index.scss');
    ...
    it('handles file + options case', function(done) {
      let plugin = new StylePlugin(fixture('index.sass'), { sass: { indentWidth: 4 } });
    ...
    it('handles file + production mode case', function(done) {
      let plugin = new StylePlugin([fixture('src/custom.sass')], 'production');
    ...
    it('handles file + mode + options case', function(done) {
      let plugin = new StylePlugin('src/index.scss', process.env.NODE_ENV, { sass: { indentWidth: 4 } });
      expect(plugin.options.sourceMap).to.be.true;
      expect(plugin.options.sass.indentedSyntax).to.be.true;
  ...
  describe('#apply', function() {
    it('is used in compile mode', function(done) {
      compile(new StylePlugin(fixture('loading.scss'), 'production', { sass: { includePaths: [path.resolve(__dirname, '..', 'node_modules')]} }), function(stats, fs, compilation) {
    ...
    it('adds source maps if they were set', function(done) {
      compile(new StylePlugin(fixture('page.sass')), function(stats, fs, compilation) {
        expect(stats.errors).to.be.empty;
    ...
    it('catches render errors', function(done) {
      compile(new StylePlugin(fixture('invalid.scss')), function(stats) {
  ...
  describe('#apply + watch', function() {
    it('audits style dependecies', function(done) {
      watch(new StylePlugin({ [fixture('page.sass')]: 'index.css' }), function(stats, fs, compilation) {
        expect(stats.errors).to.be.empty;
